...
A_ra = generate_dictionary(zr,zt,g,P+1);
C_ra = generate_clutter(zr,zt,201,P+1);
...
% Pdu = [0.9425 0.9695 0.9856 0.9944 0.998 0.9993 0.9999 1]';
% Pfu = [0.001 0.0032 0.0071 0.0199 0.053 0.1421 0.3733 0.8495]';
...
% [S_AMF1,P1] = CFAR_AMF(y_ula1,A_ula1,Z_ula1,fau*10);
[S_AMF2,P2] = CFAR_AMF(y_ula2,A_ula2,Z_ula2,fau);
% [S_AMF3,P3] = CFAR_AMF(y_ra,A_ra,Z_ra,fau*10);
% S_MP = CFAR_MHMP4(y_ra,A_ra,[],fau,Z_ra);
% S_MBMP = CFAR_MHMP4(y_ra,A_ra,[3 3 1],fau,Z_ra);

% Record results

for f = 1:length(fa)
    
    Pd(f,:) = Pd(f,:) + [length(intersect(S_AMF1(f).S,S)) length(intersect(S_AMF2(f).S,S)) length(intersect(S_AMF3(f).S,S)) length(intersect(S_MP(f).S,S)) length(intersect(S_MBMP(f).S,S))];
    Pf(f,:) = Pf(f,:) + 1 - [isempty(setdiff(S_AMF1(f).S,S)) isempty(setdiff(S_AMF2(f).S,S)) isempty(setdiff(S_AMF3(f).S,S)) isempty(setdiff(S_MP(f).S,S)) isempty(setdiff(S_MBMP(f).S,S))]; 
    
end
      
if isempty(setdiff(S_MP(6).S,S)) > isempty(setdiff(S_MBMP(6).S,S))
    clc
end
...
% figure, semilogx(Pf(:,1),Pd(:,1),Pf(:,3),Pd(:,3),'-ks',Pf(:,2),Pd(:,2),'-rx',Pf(:,4),Pd(:,4),'-go',Pf(:,5),Pd(:,5),'-m+','LineWidth',2,'MarkerSize',10)
% xlabel('P_F')
% ylabel('P_D')
