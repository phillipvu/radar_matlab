...
for f = 1:length(fa)
    ...
    Pd(f,:) = Pd(f,:) + [length(intersect(S_AMF2(f).S,S)) length(intersect(S_MP(f).S,S)) a];
    Pf(f,:) = Pf(f,:) + 1 - [isempty(setdiff(S_AMF2(f).S,S)) isempty(setdiff(S_MP(f).S,S)) (b==0)];  
